...
age_climate_housing <- as.formula("BTUTOT~age05+age05to09+age10to14+age15to19+age20to24+age25to29+
                                       age30to34+age35to39+age40to44+age45to49+age50to54+age55to59+
                                       age60to64+age65to69+age70to74+age75to79+age80p+
                                       HDD65+CDD65+
                                       TYPEHUQ+HOMEAREA+YEARMADE10"
...
out5 <- model("age_climate_housing",1987)
detach(d87)
attach(d90)
out6 <- model("age",1990)
out7 <- model("age_climate",1990)
out8 <- model("age_climate_econ",1990)
out9 <- model("age_climate_econ_housing",1990)
out10 <- model("age_climate_housing",1990)
detach(d90)
attach(d09)
out11 <- model("age",2009)
out12 <- model("age_climate",2009)
out13 <- model("age_climate_econ",2009)
out14 <- model("age_climate_econ_housing",2009)
out15 <- model("age_climate_housing",2009)

detach(d09)

## joining the results
OUTPUT <- rbind(out1,out2,out3,out4,out5,out6,out7,out8,out9,out10,out11,out12,out13,out14,out15)
rm(out1,out2,out3,out4,out5,out6,out7,out8,out9,out10,out11,out12,out13,out14,out15)
